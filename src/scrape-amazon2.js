...
async function getName(page) {
    ...
    return await page.evaluate((el) => el.innerText, title);
}
...
async function scrape(page, urlTemplate, fnsku) {
...
async function getItemInfo(browser, csvParser, itemIn) {
    ...
    try {
        const urlTemplate = "https://www.amazon.ae/dp/{fnsku}?language=en_AE";
        const urlTemplateArabic = "https://www.amazon.ae/dp/{fnsku}?language=ar_AE";
        
        const itemEng = await scrape(page, urlTemplate, itemIn.ASIN);
        ...
        if (err.name == "TimeoutError") {
            ...
            page.close();
            await getItemInfo(browser, csvParser, itemIn);
        } else {
            console.error(err);
        }
    }
